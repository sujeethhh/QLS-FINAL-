...
export default function CoursePageEnhanced({ courseData }) {
  ...
  const wordSlideVariants = {
    ...
    visible: { opacity: 1, x: 0, y: 0, transition: { type: "spring", stiffness: 100, damping: 12, duration: 0.6 } }
  };

  const headingVariants = {
    hidden: { opacity: 0, y: 40, rotateX: -15 },
    visible: { opacity: 1, y: 0, rotateX: 0, transition: { type: "spring", stiffness: 120, damping: 20, duration: 1 } }
  };
  ...
  const imageVariants = {
    ...
    visible: { opacity: 1, scale: 1, rotateY: 0, transition: { type: "spring", stiffness: 100, damping: 15, duration: 1.2 } }
  };

  // Handle both string and array overview
  const overviewParagraphs = Array.isArray(courseData.overview)
    ? courseData.overview
    : [courseData.overview];
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div
              ...
              role="img"
              aria-label={`${courseData.title} background`}
            />
            <motion.div
              ...
              <div className="relative">
                ...
                >
                  <div className="text-center">
                    ...
                    <div className="text-xs text-slate-600 font-medium">{courseData.tagline || "Learn. Certify. Excel."}</div>
                  </div>
                </motion.div>
              </div>
            </motion.div>
        ...
        <section className="w-full bg-white py-16 px-4 md:px-8">
          <div className="container mx-auto max-w-7xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {courseData.relatedCourses.map((course, idx) => (
                <motion.div
                  key={idx}
                  initial={{ opacity: 0, y: 20 }}
                  whileInView={{ opacity: 1, y: 0 }}
                    ...
                    <div className="bg-white rounded-xl shadow-md hover:shadow-xl transition-all duration-300 overflow-hidden border border-gray-100">
                      <div className="relative h-48 overflow-hidden">
                        ...
                        <div className="absolute top-3 left-3">
                          <span className={`${course.badgeColor || "bg-brand-primary"} text-white text-xs font-semibold px-3 py-1 rounded-full`}>
                            {course.badge}
                          </span>
                        </div>
                      </div>
